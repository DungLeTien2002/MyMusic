...
fun parseMoodsMomentObject(data: BrowseResponse?): MoodsMomentObject? {
    if (data != null) {
        ...
        val items =
            data.contents?.singleColumnBrowseResultsRenderer?.tabs?.get(0)?.tabRenderer?.content?.sectionListRenderer?.contents
...
fun parseGenreObject(data: BrowseResponse?): GenreObject? {
    if (data != null) {
        ...
        if (items != null) {
            for (item in items) {
                if (item.musicCarouselShelfRenderer != null) {
                    ...
                    val listContent: MutableList<com.example.mymusic.base.data.models.explore.mood.genre.Content> =
                        mutableListOf()
                    if (!contents.isNullOrEmpty()) {
                        for (content in contents) {
                            if (content.musicResponsiveListItemRenderer != null) {
                                ...
                                val playlistBrowseId =
                                    content.musicTwoRowItemRenderer.navigationEndpoint.browseEndpoint?.browseId
                                listContent.add(
                                    com.example.mymusic.base.data.models.explore.mood.genre.Content(
                                        playlistBrowseId = playlistBrowseId ?: "",
        ...
        return GenreObject(
            header = title,
            itemsPlaylist = listItemsPlaylist,
            itemsSong = listItemsSong
        )
    } else {
        return null
    }
